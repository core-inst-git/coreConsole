...
function formatAxisValue(val: number): string {
  // Keep ~4 significant figures so adjacent y ticks over a narrow span (e.g.
  // 110.5 / 111.0 uW) don't both round to the same integer label.
  const a = Math.abs(val);
  if (a === 0) return '0';
  if (a >= 1000) return val.toFixed(1);
  if (a >= 100) return val.toFixed(1);
  if (a >= 10) return val.toFixed(2);
  ...
  useEffect(() => {
    ...
    const option: echarts.EChartsOption = {
      ...
      xAxis: {
        ...
        axisLabel: {
          color: '#8B94A3',
          hideOverlap: true,        // drop labels that would collide (was: 8308.0s8309.0s...)
          showMaxLabel: false,
          formatter: (v: number) => `${v.toFixed(1)}s`,
          ...(compact ? { fontSize: 10, margin: 8 } : { margin: 8 })
        },
