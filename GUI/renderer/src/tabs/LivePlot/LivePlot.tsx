...
const RECORD_DEFAULT_S = 10; // 0 = record until Stop
...
export default function LivePlot({
  ...
  const [recDurationS, setRecDurationS] = useState<number>(() => {
    try {
      ...
      return Number.isFinite(v) && v >= 0 ? v : RECORD_DEFAULT_S;
    } catch {
      return RECORD_DEFAULT_S;
    }
  });
  ...
  useEffect(() => {
    if (!recording) return;
    const iv = window.setInterval(() => setRecElapsed((p) => p + 1), 1000);
    return () => window.clearInterval(iv);
  }, [recording]);

  // Lock tab switching while recording (parent disables nav until Stop).
  useEffect(() => {
  ...
  useEffect(() => {
    return subscribeControl((msg: ControlMsg) => {
      if (msg.action === 'record_start' && !msg.ok) {
        ...
        setRecording(false);
        if (msg.ok) {
          const parts = Number(msg.parts ?? 1);
          setRecNote(
            `Saved ${msg.frames ?? '?'} frames in ${parts} file${parts === 1 ? '' : 's'} → ${msg.path ?? ''}`
          );
        } else {
          setRecNote(`Record failed: ${msg.error || 'unknown error'}`);
        }
  ...
  const startRecording = async () => {
    ...
    const d = Math.max(0, Math.round(recDurationS)); // 0 = until Stop
    ...
    setRecording(true);
    setRecElapsed(0);
    sendControl({ action: 'record_start', path, duration_s: d > 0 ? d : null, cards });
  };
      ...
      <div className="live-header">
        ...
        <div className="live-actions">
          ...
          <div
            ...
            title="Record open cards to rolling ~10 MB HDF5 chunks (500 Hz). 0 s = until Stop."
          >
            <input
              ...
              onBlur={() => setRecDurationS((p) => Math.max(0, Math.round(p) || 0))}
            />
            <span className="record-unit">{recDurationS > 0 ? 's' : 's (∞)'}</span>
            {!recording ? (
              <button
              ...
              >
                {recDurationS > 0
                  ? `■ Stop (${Math.max(0, recDurationS - recElapsed)}s)`
                  : `■ Stop (${recElapsed}s)`}
              </button>
            )}
          </div>
