...
function loadSweepSettings(): SweepSettings {
  try {
    ...
    for (const k of ['startNm', 'stopNm', 'speedNmS', 'powerMw', 'sampleRateHz', 'defaultWlNm'] as const) {
...
export default function CaptureTab({
  ...
  const osIdx = 1; // oversampling fixed at index 1 (2x); UI control removed
  ...
  useEffect(() => {
    saveSweepSettings({ startNm, stopNm, speedNmS, powerMw, sampleRateHz, defaultWlNm });
  }, [startNm, stopNm, speedNmS, powerMw, sampleRateHz, defaultWlNm]);
  ...
  const warnUser = (message: string) => {
    addLog(message);
    // In-app banner instead of window.alert: a native modal on Windows
    // steals keyboard focus from the web view and doesn't return it,
    // which was killing all typing (incl. Laser Power) after any error.
    setBanner(message);
  };
