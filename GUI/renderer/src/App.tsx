...
export default function App() {
  ...
  // Recording locks tab switching until the user stops it (recording lives on
  // the Live tab; other tabs would unmount its stream cards mid-record).
    ...
    const unsub = subscribeStatus((s) => {
      ...
      if (unsupported?.unsupported_firmware) {
        if (!unsupportedPopupShown.current) {
          ...
          console.warn(`Device not supported: ${reason}${idnSuffix}`);
        ...
        <nav className="sidebar">
          ...
          {tabs.map((t) => (
            <button
              ...
              disabled={isRecording && t.id !== activeTab}
              title={isRecording && t.id !== activeTab ? 'Stop recording to switch tabs' : undefined}
            >
              {t.label}
            </button>
          ))}
