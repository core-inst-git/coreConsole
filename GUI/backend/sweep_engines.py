...
ARM_SETTLE_S = 0.8
# Grace beyond the pure sweep time for the laser to slew to the start
# wavelength and re-level after a power change before it fires the
# sweep-start trigger. A fixed wait + single readiness check produced false
# "Acquisition did not complete" errors when the power was changed.
LASER_START_GRACE_S = 25.0
COMPLETION_POLL_S = 0.2
...
def run_continuous_sweep(dev: Any, laser: TunableLaser, p: SweepParams, *,
    ...
    try:
        ...
        # Poll for completion rather than waiting a fixed time and checking
        # once: the laser may fire its sweep-start trigger late (slew to start
        # wavelength, re-level after a power change), so the acquisition can
        # finish well after the nominal sweep duration.
        sweep_s = n_frames / p.sample_rate_hz
        deadline = time.monotonic() + sweep_s + SWEEP_TRANSFER_OVERHEAD_S + LASER_START_GRACE_S
        ready = False
        while time.monotonic() < deadline:
            time.sleep(COMPLETION_POLL_S)
            try:
                if dev.capture_is_data_ready():
                    ready = True
                    break
            except Exception:
                pass
        ...
        if not ready:
            ...
            raise LaserError(
                ...
                f"{sweep_s + SWEEP_TRANSFER_OVERHEAD_S + LASER_START_GRACE_S:.0f} s — "
                "the laser may not have fired its sweep-start trigger. Check the "
                "trigger BNC wiring (laser trigger out -> coreDAQ trigger in) and "
                "that the laser output is enabled")
