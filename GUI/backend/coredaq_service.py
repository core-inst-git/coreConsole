...
# Oversampling is fixed app-wide (the user-facing selector was removed).
FIXED_OS_IDX = 1
...
class Session:
    ...
    def __init__(self, device_id: str, port: str, dev: coreDAQ) -> None:
        ...
        self.default_os_idx = FIXED_OS_IDX
...
class CoreDAQBackend:
    def __init__(self, port_override: Optional[str] = None, timeout_s: float = 0.2,
        ...
        # stream loop and writes an HDF5 file when the window elapses (<=360 s).
        # Chunked recorder state (rolling ~10 MB HDF5 files); see _start_record.
    ...
    async def _stream_loop_iteration(self) -> None:
        if not self.stream_enabled_global or not self.devices:
            ...
            if self.record is not None and (
                ...
                or not self.devices
                or self._record_duration_elapsed()
            ):
        ...
        # Roll to a new chunk file once the current one hits the size cap, so
        # arbitrarily long recordings never grow memory or a single huge file.
        if self.record is not None and self._record_part_full():
            await self._flush_record_part(final=False)

        if self.record is not None and (
            self.record.get("stop") or self._record_duration_elapsed()
        ):
            await self._finalize_record()
    ...
    async def _handle_control(self, ws: Any, data: dict) -> None:
        ...
        try:
            ...
            if action == "set_os":
                # Oversampling is fixed at FIXED_OS_IDX; ignore any requested
                # value (the UI control was removed).
                sess = self._get_session(requested_id)
                sess.default_os_idx = FIXED_OS_IDX
                await asyncio.to_thread(sess.dev.set_oversampling, FIXED_OS_IDX)
                await ws.send(json.dumps({"type": "control", "action": action, "ok": True,
                                          "error": None, "device_id": sess.device_id}))
                return
    ...
    # ~10 MB per chunk file (override with COREDAQ_RECORD_PART_BYTES for tests).
    RECORD_PART_MAX_BYTES = int(os.environ.get("COREDAQ_RECORD_PART_BYTES", 10 * 1024 * 1024))
    _RECORD_BYTES_PER_FRAME_DEVICE = 8 + 4 * 8  # time + 4 float64 channels

    def _record_duration_elapsed(self) -> bool:
        rec = self.record
        if rec is None:
            return False
        t_end = rec.get("t_end")
        return t_end is not None and now_sec() >= t_end

    def _record_part_full(self) -> bool:
        rec = self.record
        if rec is None:
            return False
        longest = max((len(b["t"]) for b in rec["buffers"].values()), default=0)
        return longest >= rec["part_max_frames"]

    def _start_record(self, data: dict) -> dict:
        """Validate a record_start request and install the recorder state.

        Recording streams into a series of ~10 MB HDF5 chunk files
        (``<name>_0001.h5``, ``_0002.h5``, ...), so it can run for an
        arbitrary duration with flat memory. ``duration_s`` is optional:
        omit it (or 0) to record until the user hits Stop.
        """
        ...
        # Optional duration: None/0 -> record until stopped. No upper cap.
        raw_dur = data.get("duration_s")
        duration_s: Optional[float]
        if raw_dur in (None, "", 0, 0.0):
            duration_s = None
        else:
            try:
            ...
            if duration_s < 0.5:
                raise coreDAQError("Duration must be at least 0.5 s (or blank for until-stop)")
        ...
        # Frames per chunk so each file stays under ~10 MB (device raw + one
        # value dataset per card).
        per_frame = (len(needed_devices) * self._RECORD_BYTES_PER_FRAME_DEVICE
                     + len(cards) * 2 * 8)
        part_max_frames = max(1000, int(self.RECORD_PART_MAX_BYTES / max(per_frame, 1)))
        ...
        self.record = {
            ...
            "started": started,
            "t_end": (started + duration_s) if duration_s else None,
            "cards": cards,
            "buffers": {d: {"t": [], "ch": [[], [], [], []]} for d in needed_devices},
            "part_max_frames": part_max_frames,
            "part_index": 0,
            "parts": [],
            "total_frames": 0,
            "write_tasks": [],
        ...
        return {"path": base, "duration_s": duration_s,
                "part_max_frames": part_max_frames, "chunked": True}
    ...
    async def _write_part_task(self, rec: dict, part: dict) -> None:
        try:
            frames = await asyncio.to_thread(self._write_record_part, part)
            if frames:
                rec["total_frames"] += frames
                rec["parts"].append(part["path"])
                log(f"recording chunk {part['part_index']}: {frames} frames -> {part['path']}")
        except Exception as err:
            rec["write_error"] = str(err)
            log(f"recording chunk write FAILED: {err}")

    async def _flush_record_part(self, *, final: bool) -> None:
        """Detach the current buffer into the next chunk file. Non-final chunks
        are written on a background task so the 500 Hz sampling loop never
        pauses (no gap at chunk boundaries); the final chunk is awaited."""
        rec = self.record
        if rec is None:
            return
        longest = max((len(b["t"]) for b in rec["buffers"].values()), default=0)
        if longest == 0:
            return  # nothing buffered (e.g. final flush after an exact roll)
        rec["part_index"] += 1
        part = {
            "path": f"{rec['base']}_{rec['part_index']:04d}.h5",
            "part_index": rec["part_index"],
            "started": rec["started"],
            "cards": rec["cards"],
            "device_meta": rec["device_meta"],
            "buffers": rec["buffers"],
            "nominal_hz": self.RECORD_NOMINAL_HZ,
            "final": final,
        }
        # Detach the filled buffers; new samples accumulate in fresh lists.
        rec["buffers"] = {d: {"t": [], "ch": [[], [], [], []]} for d in rec["buffers"]}
        if final:
            await self._write_part_task(rec, part)
        else:
            rec["write_tasks"].append(
                asyncio.create_task(self._write_part_task(rec, part)))
    ...
    async def _finalize_record(self) -> None:
        ...
        if rec is None:
            return
        self.record = None
        # Restore stream flags synchronously, before any await.
        ...
        try:
            # Temporarily restore rec so _flush_record_part can write the tail.
            self.record = rec
            await self._flush_record_part(final=True)
            self.record = None
            # Drain any in-flight background chunk writes.
            pending = [t for t in rec["write_tasks"] if not t.done()]
            if pending:
                await asyncio.gather(*pending, return_exceptions=True)
            if rec["write_error"] and not rec["parts"]:
                raise coreDAQError(f"Chunk write failed: {rec['write_error']}")
            if not rec["parts"]:
                raise coreDAQError("Recording captured no samples (stream stalled?)")
            parts_sorted = sorted(rec["parts"])
            await self.broadcast({
                "type": "control", "action": "record_done", "ok": True,
                "error": rec["write_error"],
                "path": parts_sorted[0], "parts": len(parts_sorted),
                "frames": int(rec["total_frames"]), "cards": len(rec["cards"]),
            })
            log(f"recording saved: {len(parts_sorted)} chunk(s), "
                f"{rec['total_frames']} frames, base {rec['base']}")
        except Exception as err:
            self.record = None
    ...
    def _write_record_part(self, part: dict) -> int:
        """Write one chunk file (blocking; runs in a worker thread). Returns
        the frame count. Time is relative to the recording start, so parts
        share a continuous time axis."""
        import numpy as np
        import h5py
        ...
        for did, buf in buffers.items():
            dev_t[did] = np.asarray(buf["t"], dtype=np.float64) - started
            dev_ch[did] = np.asarray(buf["ch"], dtype=np.float64)  # (4, N)
        ...
        with h5py.File(part["path"], "w") as f:
            f.attrs["created_utc"] = iso_utc_now()
            f.attrs["app"] = "coreConsole"
            f.attrs["source"] = "live_record"
            f.attrs["nominal_sample_rate_hz"] = float(part["nominal_hz"])
            f.attrs["part_index"] = int(part["part_index"])
            f.attrs["final_part"] = bool(part["final"])
            ...
            for idx, card in enumerate(part["cards"]):
                ...
                if card["kind"] == "physical":
                    ...
                    if card["src_a"][0] not in dev_t or card["src_b"][0] not in dev_t:
                        continue
