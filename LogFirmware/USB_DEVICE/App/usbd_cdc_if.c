...
static void process_cmd(char *line)
{
    ...
    if (strcmp(s, "IDN?") == 0)     { send_line("OK ", "CoreDAQ_Mk1_InGaAs_LOG_FW_v4.0_SNX0002"); return; }
    ...
    // LOGCAL <head>
    if (s[0]=='L' && s[1]=='O' && s[2]=='G' && s[3]=='C' && s[4]=='A' && s[5]=='L' &&
        (s[6]==0 || s[6]==' ' || s[6]=='\t'))
    {
        const char *p = s + 6;
        while (*p==' ' || *p=='\t') ++p;
        ...
        uint32_t head = 0;
        if (!parse_u32(p, &head) || head < 1 || head > 4) {
            send_line("ERR ", "BAD_PARAM");
            return;
        }

        // arm state machine
        CALDUMP_Start((uint8_t)head);
        // Optionally ACK
        SM_SetState(STATE_CAL_DUMP);
        send_line("OK ", "CAL_DUMP_ARMED");
        return;
    }
