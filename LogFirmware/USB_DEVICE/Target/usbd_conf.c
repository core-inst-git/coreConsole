...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
  /* Larger HS FIFOs help long CDC bulk transfers remain stable. */
  HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_HS, 512);
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 0, 64);   /* EP0 */
  HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_HS, 1, 512);  /* CDC IN */
  }
  return USBD_OK;
}
